...
class Client(ThirdPartyInterface):
    def __init__(self):
        ...
        self.url = BASE_URL
        self.verify = VERIFY
        self.encoded_string = base64.b64encode('{}:{}'.format(CLIENT_ID, API_KEY).encode()).decode()
    ...
    def _oauth(self):
        self.logger.debug('encoded_string:{}'.format(self.encoded_string))
        auth_header = {
            'Authorization': 'Basic ' + self.encoded_string,
            'Content-Type': 'application/x-www-form-urlencoded',
            'User-Agent': 'VectraActiveEnforcement2.0'
        }
        self.logger.debug('{}'.format(auth_header))
        body = 'grant_type=client_credentials&scope=connect.api.write&scope=connect.api.read'
        r = requests.post(self.url + '/as/token.oauth2', headers=auth_header, data=body, verify=self.verify)
        if r.ok:
            self.oauth_expiry = datetime.now() + timedelta(seconds=r.json()['expires_in'] - 60)
    ...
    def _host_search(self, search_params={}):
        host_list = []
        next_anchor = False
        params = {**search_params, **{'organizationId': self.organization_id, 'type': 'computer'}}
        r = requests.get(self.url + '/devices/v1/devices',
        ...
        if r.ok:
            ...
            anchor = False
            self.logger.error('Unable to retrieve host, query: {}, error:{}'.format(anchor, r.content))
    ...
    def _search_computer(self, host: VectraHost) -> list:
        ...
        if len(results) > 0:
            ...
            for item in results:
                ...
                # Search via MACs
                mac_addresses = [mac.lower().replace('-', '') for mac in item.get('macAddresses', '').split()]
            ...
            if host_ip_list:
                ...
                if host_ip_mac_list:
                    ...
                    return [i.get('id') for i in host_mac_list] if host_mac_list else [i.get('id') for i in host_ip_list]
            else:
    ...
    def _set_isolation(self, comp_id, action, host: VectraHost):
        ...
        if action == 'isolateFromNetwork':
            body['parameters'] = {'message': 'Your device will be isolated by Vectra, Host ID: {}'.format(host.id)}

        results = requests.post(self.url + '/devices/v1/operations',
                                headers=self.headers,
                                json=body,
                                verify=self.verify)
        if results.ok:
            ...
            self.logger.error('isolation request {} failed, response:{}'.format(action, results.content))
        return results
    ...
    def block_host(self, host: VectraHost) -> list:
        ...
        if len(computers) == 1:
            self.logger.info('Requesting WithSecure isolation of computer with ID {} and IP {}'.format(
                computers[0], ip_address
            ))
            results = self._set_isolation(computers[0], 'isolateFromNetwork', host)
            if results.ok:
                self.logger.info('Successfully  isolation of computer with agent ID {} name {}and IP {}'.format(
                    computers[0], host.name, ip_address
                ))
                return [computers[0]]
            else:
                self.logger.error('Unable to isolate computer with ID {} and IP {}. {}'.format(
                    computers[0], ip_address, results.content
                ))
                return []
        elif len(computers) == 0:
            self.logger.info('No computers ({}) were found with IP: {}, name: {}, MACs: {}'.format(
                len(computers), ip_address, host.name, host.mac_addresses
            ))
            return []
        else:
            self.logger.info('More than 1 computers ({}) were found with IP: {}, name: {}, MACs: {}'.format(
                len(computers), ip_address, host.name, host.mac_addresses
            ))
            return []
    ...
    def unblock_host(self, host: VectraHost) -> list:
        ...
        if len(comp_ids) >= 1:
            for comp_id in comp_ids:
                self.logger.info('Requesting WithSecure un-isolation of computer with ID {} and IP {}'.format(
                    comp_id, host.ip
                ))
                results = self._set_isolation(comp_id, 'releaseFromNetworkIsolation', host)

                if results.ok:
                    self.logger.info('Successfully  un-isolation of computer with ID {} and IP {}'.format(
                        comp_id, host.ip
                    ))
                    un_isolated.append(comp_id)
                else:
                    self.logger.info('Unable to un-isolate computer with ID {} and IP {}. {}'.format(
