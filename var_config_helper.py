...
try:
    for conf in confs:
        if conf == "third_party_clients".upper():
            ...
            if (
                conf in ["SMTP_PORT", "SRC_EMAIL", "DST_EMAIL", "SMTP_AUTH"]
                and confs["SMTP_SERVER"] == ""
            ):
                pass
                if (
                    ...
                    pass
            if (
                conf in ["SYSLOG_PORT", "SYSLOG_PROTO", "SYSLOG_FORMAT"]
                and confs["SYSLOG_SERVER"] == ""
            ):
                pass
...
for client in clients:
    confs = {}
    client = client.strip('"')
    file = glob(f"third_party_client_configs/{client}/*_config.py")
    file = file[0]
